...
if DOF >= 1
    ...
    [chi1_energy] = get_energy_wProtein(Clash_lists.c1_Clash, Position,  [], []);
    total_energy = total_energy +chi1_energy;
    
    if DOF >= 2
        [chi2_energy] = get_energy_wProtein(Clash_lists.c2_Clash, Position,  [], []);
        total_energy = total_energy + chi2_energy;
    end
    if DOF >= 3
        [chi3_energy] = get_energy_wProtein(Clash_lists.c3_Clash, Position,  [], []);
        total_energy = total_energy + chi3_energy;
    end
end
if CH3 == 2
    [HG2_energy] = get_energy_wProtein(Clash_lists.HG2_Clash, Position,  [], []);
    total_energy = total_energy + HG2_energy;
end
if CH3 >=1
    [HG1_energy] = get_energy_wProtein(Clash_lists.HG1_Clash, Position,  [], []);
    total_energy = total_energy + HG1_energy;
end
if OH ==1
    [OH_energy] = get_energy_wProtein(Clash_lists.OH_Clash, Position,  [],[]);
